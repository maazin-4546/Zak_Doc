...
const extractInvoiceData = async (imagePath) => {
    try {
        ...
        let responseText = result.response.text(); // Get API response text

        // 🛠 Extract JSON from response using regex
        const jsonMatch = responseText.match(/```json\s*([\s\S]*?)\s*```/);
        if (jsonMatch) {
            responseText = jsonMatch[1]; // Extract only the JSON part
        }

        const extractedData = JSON.parse(responseText.trim()); // ✅ Parse cleaned JSON
...
// API Endpoint to handle image upload and extraction
